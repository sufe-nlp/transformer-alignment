...
class SequenceNpad(object):
    ...
    def prepare_pdic(self, phrase, src_tokens, tokenizer, bpe, target=None, cons_type='bpe'):
        count=1
        p_dict={}
        p_dict[0]=[]
        ...
        for item in phrase:
            if 'tgt_span' in item:
                ...
                if cons_type == 'raw':
                    item['src'] = self.encode_fn(item['src'], tokenizer, bpe)
                    item['tgt'] = self.encode_fn(item['tgt'], tokenizer, bpe)
                src_p = self.tgt_dict.encode_line(item['src'], add_if_not_exist=False, append_eos=False).type_as(src_tokens)
                tgt_p = self.tgt_dict.encode_line(item['tgt'], add_if_not_exist=False, append_eos=False).type_as(src_tokens)
    ...
    def encode_fn(self, x, tokenizer, bpe):
    ...
    def _decode(self, tokens, encoder_outs, incremental_states=None, npad=False):
        if len(self.models) == 1:
            return self._decode_one(tokens, self.models[0], encoder_outs[0], incremental_states, log_probs=True, npad=npad)
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs, npad=False):
        alignment_heads=1 if self.args.alignment_task=='usehead' else None

        with torch.no_grad():
            if incremental_states is not None and incremental_states[model] is not None:
                decoder_out = list(model.decoder(tokens, encoder_out, incremental_state=incremental_states[model], alignment_heads=alignment_heads))
            elif npad:
                decoder_out = list(model.decoder(tokens, encoder_out, alignment_heads=alignment_heads, npad=True))
            else:
                decoder_out = list(model.decoder(tokens, encoder_out, alignment_heads=alignment_heads))
    ...
    def npad_decode(self, encoder_input, maxlen_r=1.5, beam_size=1):       
        src_tokens = encoder_input['src_tokens']
        bsz, srclen = src_tokens.size()
        ...
        tokens = src_tokens.data.new(bsz * beam_size, maxlen*3).fill_(self.pad)
        scores = src_tokens.data.new(bsz * beam_size, maxlen + 1).float().fill_(0)

        tokens[:, 0] = self.eos
        eos_yet = src_tokens.new_zeros(bsz*beam_size, 1).bool() 
        
        for step in range(maxlen + 1):  # one extra step for EOS marker
            for model in self.models:
                assert incremental_states[model] is None

            lprobs, _ = self._decode(tokens[:, :step + 1], encoder_outs, incremental_states, npad=True)
            lprobs[:, self.pad] = -math.inf  # never select pad
            lprobs[:, self.unk] -= self.unk_penalty  # apply unk penalty

            _, next_word = torch.max(lprobs, dim = -1)
            next_word = next_word.unsqueeze(dim=-1)
            next_word[eos_yet] = self.pad
            tokens[:,step + 1] = next_word.squeeze(dim=-1) 
            mx = lprobs.gather(dim=-1, index=next_word)
            mx[eos_yet] = 0 
            scores[:,step] = mx.squeeze(dim=-1)  
            eos_yet = eos_yet | (next_word == self.eos)
            if eos_yet.all():
                break
        
        step = step + 1
        # import pdb;pdb.set_trace()
        lprobs, _ = self.models[0].decoder(tokens[:, :step + 1], encoder_outs[0], npad=False)       
        lprobs = F.log_softmax(lprobs.float(), dim=-1)
        lprobs[:, :, self.pad] = 0
        ppl = lprobs.gather(dim=-1, index=tokens[:, :step + 1].unsqueeze(dim=-1)).sum(dim=1).squeeze(dim=-1)

        tgt_lengths =  (tokens[:, 1:step + 1] != self.pad).int().sum(dim=-1)
        # coeff = ((5 + tgt_lengths) / 6.0) ** 0.1
        ppl = ppl / tgt_lengths
        _, index = ppl.max(dim=-1)

        final = tokens[index].clone()
        final[:-1] = tokens[index,1:]
        final[-1] = self.pad 
